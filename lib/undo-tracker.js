...
export default class UndoTracker {
  ...
  onStackSizeChange() {
    this.onRangeChange(this.undoStack, this.undoStackSize + this.redoStackSize);
  }
  ...
  setPosition = () => {
        if (this.activeEditor.buffer.historyProvider.undoStack) {
          this.undoStackSize = this.activeEditor.buffer.historyProvider.undoStack.length;
        }
  ...
  changePosition = (newPosition) => {
    if (this.activeEditor && newPosition) {
